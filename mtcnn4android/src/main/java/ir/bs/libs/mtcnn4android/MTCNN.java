...
public class MTCNN {
    ...
    private int PNetForward(Bitmap bitmap, float[][] PNetOutProb, float[][][] PNetOutBias) {
        ...
        m4aUtils.flip_diag(PNetIn, h, w, 3); //沿着对角线翻转
        ...
        //【写法一】先翻转，后转为2/3维数组
        m4aUtils.flip_diag(PNetOutP, PNetOutSizeW, PNetOutSizeH, 2);
        m4aUtils.flip_diag(PNetOutB, PNetOutSizeW, PNetOutSizeH, 4);
        m4aUtils.expand(PNetOutB, PNetOutBias);
        m4aUtils.expandProb(PNetOutP, PNetOutProb);
        /*
    ...
    private Vector<Box> PNet(Bitmap bitmap, int minSize) {
        ...
        return m4aUtils.updateBoxes(totalBoxes);
    }
    ...
    private Vector<Box> RNet(Bitmap bitmap, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            m4aUtils.flip_diag(curCrop, 24, 24, 3);
        ...
        return m4aUtils.updateBoxes(boxes);
    }
    ...
    private Vector<Box> ONet(Bitmap bitmap, Vector<Box> boxes) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            m4aUtils.flip_diag(curCrop, 48, 48, 3);
